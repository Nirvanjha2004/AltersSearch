...
- [ ] 1. Install dependencies and configure design tokens
  ...
  - [-] 1.5 Update font imports in `layout.tsx`
...
- [ ] 2. Add Aceternity UI primitives to `components/ui/`
  - [~] 2.1 Create `components/ui/spotlight.tsx`
  ...
  - [~] 2.2 Create `components/ui/shimmer-button.tsx`
  ...
  - [~] 2.3 Create `components/ui/placeholder-vanish-input.tsx`
  ...
  - [~] 2.4 Create `components/ui/hover-effect.tsx`
  ...
  - [~] 2.5 Create `components/ui/background-gradient.tsx`
    - Implement `<BackgroundGradient />` card wrapper with an animated gradient border
    - Accept `children` and `className` props
    - Use `cn()` for class merging
    - _Requirements: 9.7, 13.2_

  - [~] 2.6 Create `components/ui/moving-border.tsx`
...
- [ ] 3. Create mock data file
  - [~] 3.1 Create `frontend/src/data/repos.ts` with `MOCK_REPOS` constant
...
- [ ] 4. Rebuild Sign In page (`app/login/page.tsx`)
  - [~] 4.1 Implement full-viewport layout with dark background and Spotlight
