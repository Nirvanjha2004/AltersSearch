-- Migration: switch embedding column from 384 -> 768 dimensions
-- Required because we moved from all-MiniLM-L6-v2 (384d) to
-- Google gemini-embedding-001 pinned to 768d via Matryoshka.
--
...
-- 1. Drop the old embedding column and recreate at 768d
--    All other repo data (name, description, stars, etc.) is preserved.
ALTER TABLE public.repos DROP COLUMN IF EXISTS embedding;
ALTER TABLE public.repos ADD COLUMN embedding VECTOR(768);
...
-- 3. Recreate the RPC with the correct 768d signature
...
-- 4. Recreate HNSW index for fast ANN search at 768d
